        ...
        case 'V': { //Version
            if (count == 1) {
                ...
                switch (device_getType()) {
                    case DEVICE_CANSTICK: uart_writeString("0"); break;
                    case DEVICE_CANSTICK_JACK: uart_writeString("1"); break;
                    case DEVICE_CANSTICK_MINI: uart_writeString("2"); break;
...
bool command_process_extra(uint8_t *buffer, uint8_t count) {
    switch (buffer[0]) {
        ...
        case 'T': {
            if (device_getType() == DEVICE_CANSTICK_MINI) {
                if (count == 1) {
            ...
            } else { //only supported on CanStick/mini
                if (State_ExtendedError) { uart_writeString("x!"); }
                return false;
            }
        }
